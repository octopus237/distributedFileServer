...
class BriceNode(rpyc.Service):
    class exposed_DNode():

        @staticmethod
        def exposed_filequery():
            print (briceNodeConfig.nodePath)
            filelist = []                                       # Create a null file list to append data while iterating
            for (dirpath, dirnames, filename) in os.walk(briceNodeConfig.nodePath): # iterating through each folder/ file in PATH
                for file in filename:
                    tempdict = {}                               # Creating a temporary dictionary to record file attributes
                    tempdict['DN_IP'] = briceNodeConfig.nodeHost                      # Assigning values for current file to th temp dict
                    tempdict['DN_Port']  = briceNodeConfig.nodePort
...
if __name__ == "__main__":
    ...
    t1 = ThreadedServer(BriceNode, hostname=briceNodeConfig.nodeHost, port=briceNodeConfig.nodePort, protocol_config={'allow_public_attrs': True})
    ...
    subprocess.Popen("python briceFtpServer.py", shell=True) # this opens the ftpserver1.py file in a new console window (shell=True) so that we can view the FTP logs
