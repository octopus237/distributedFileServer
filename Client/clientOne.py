...
cpath = r"/home/st14/CSN-Project/Client"  # Specify the client directory
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                print ("Conencted to the FTP Server {}:{}".format(host,port)) # Tries to login FTP server using given credentials
                print ("Files in cwd:\n")   
...
def main():
    ...
    port = input("Enter the server port [Default = 8080]:")   # Requests the user to input the Port number of the Master server to be connected
    if port:
         pass                                                  # In case of user input, do nothing
    else:
        port = 8080                                           # IN case of NO User input, set default Master server port
    con=rpyc.connect(host,port)                                # Connect to the Master server IP: Port
    #con=rpyc.ssl_connect(host, port, keyfile="client.key", certfile="client.crt") 
...
#                        print (type(xdict))
            ...
            print ("Could not get any input")                # Print error if user input is out of range or not a digit
    print ("Exiting the Secure File Server\nThank you!")              # Print if Opt=0 & quit
